...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName('Truecall mgrs').enableHiveSupport().getOrCreate()
    ...
    '''
    df_trc = spark.read.parquet('hdfs://njbbvmaspd11.nss.vzwnet.com:9000/user/ZheS/TrueCall/df_trc_sampled')
    df_trc_sampled.coalesce(1)
    df_trc_sampled.cache()
    df_trc_sampled.count()
    '''

    base_path1 = "hdfs://njbbvmaspd11.nss.vzwnet.com:9000/user/jennifer/truecall/TrueCall_VMB/UTC_date=20231106/UTC_hour=00/"  
    df_trc = spark.read.option("recursiveFileLookup", "true").option("header", "true").csv(base_path1).repartition(partitionNum)
    df_trc.show(1)
    '''
    ...
    df_trc = convert_string_numerical(df_trc,String_Column+Date_Column)
    df_trc = df_trc.filter(col('end_latitude').isNotNull() ).filter(col('end_longitude').isNotNull() )
    df_trc = df_trc.withColumn("env_tag", when(col("indoor_outdoor_indicator_name") == "Indoor", "Indoor").otherwise("Outdoor"))
    #----------------------------------------------------------------------
    ...
    def convert_to_mgrs(latitude, longitude, MGRSPrecision =3):
        try:
            mgrs_value = m.toMGRS(latitude, longitude,MGRSPrecision =MGRSPrecision)
            return mgrs_value
        except:
            return None
    ...
    df_trc_mgrs = df_trc.withColumn('gridid', mgrs_udf(df_trc['end_latitude'],df_trc['end_longitude']))
    df_trc_mgrs.show(1)
    df_mgrs_feature = df_trc_mgrs.groupBy("gridid", "env_tag").agg(
                avg("rsrp_dbm").alias("rsrp_dbm"),
                avg("rsrq_db").alias("rsrq_db"),
                #avg("ul_sinr_db").alas("ul_sinr_db"), this is an empty column
